...
def ChangeBoardRepresentation(board, game,exp = False):
        ...
        # add -1 to the end of horizontal_edges and vertical_edges until they have a length of 240
        horizontal_edges = np.pad(horizontal_edges, (0, 240 - horizontal_edges.shape[0]), 'constant', constant_values=-1)
        vertical_edges = np.pad(vertical_edges, (0, 240 - vertical_edges.shape[0]), 'constant', constant_values=-1)
...
class NNetWrapper(NeuralNet):
    ...
    def train(self, examples):
        ...
        # print("updated_input_boards.shape", updated_input_boards.shape)
        # print("target_pis.shape", np.asarray(target_pis).shape)
        # print("target_vs.shape", np.asarray(target_vs).shape)
        # target_pis has shape (10,112), change the shape in order to have a shape of (10,480) by using padding
        # print("target_pis.shape", np.asarray(target_pis).shape)
        # print(target_pis)
        # print(np.asarray(target_pis).shape[1])
        target_pis = [np.pad(p, (0, 480 - np.asarray(target_pis).shape[1]), 'constant', constant_values=0) for p in target_pis]
        # print("target_pis.shape", np.asarray(target_pis).shape)
        # print(target_pis)
